...
static void cpu_process(const float* in, float* out, float k, int start, int end) {
  for (int i = start; i < end; ++i) out[i] = in[i] * k + 1.0f;
}
...
int main(int argc, char** argv) {
  ...
  // Split: CPU половина, GPU половина (можно менять)
  const int split = N / 2;
  const int N_gpu = N - split;

  std::cout << "Task 3: Hybrid CPU+GPU profiling (cudaMemcpyAsync + streams)\n";
  std::cout << "Op: out[i] = in[i]*k + 1\n";
  std::cout << "N=" << N << " k=" << k << " split=" << split
            << " (CPU " << split << ", GPU " << N_gpu << ")\n\n";

  // Оптимизация: pinned host memory 
  // Асинхронные копии работают лучше (и честнее) с pinned memory
  ...
  for (int i = 0; i < N; ++i) {
    h_in[i] = (float)((i % 1000) / 1000.0);   // детерминированно, без rand()
    h_ref[i] = h_in[i] * k + 1.0f;
  }

  // Device buffers только под GPU-часть
