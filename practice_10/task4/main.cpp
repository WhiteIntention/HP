...
struct Times {
    double gen = 0;       // генерация/подготовка локальных данных
    double comp = 0;      // локальные вычисления
    double comm = 0;      // коммуникации (reduce/allreduce)
    double total = 0;     // полный тайминг (gen+comp+comm с барьерами)
};
...
int main(int argc, char** argv) {
    ...
    // Аргументы:
    // mode: strong | weak
    // reduce_mode: reduce | allreduce
    // N: для strong это общий размер, для weak это размер на процесс
    ...
    // strong: глобальный N фиксирован, local_N = ceil(N/size)
    // weak: local_N фиксирован, глобальный N растёт: N = local_N * size
    ...
    if (mode == "weak") {
        ...
        local_N = (global_N + size - 1) / size; // ceil
    }

    long long start = (long long)rank * ((mode=="weak") ? local_N : (global_N + size - 1) / size);
    ...
    auto run_once = [&](Times& T){
        ...
        // 1) Генерация локальных данных
        double tg0 = MPI_Wtime();
        std::mt19937_64 gen(1234 + rank);
        std::uniform_real_distribution<double> dist(0.0, 1.0);
        for (long long i = 0; i < real_local_N; ++i) a[(size_t)i] = dist(gen);
        ...
        // 3) Коммуникации: Reduce или Allreduce
    ...
    if (rank == 0) {
        ...
        std::cout << "MPI Scaling (aggregate: sum/min/max)\n";
        std::cout << "Mode = " << mode << " | Comm = " << rmode << " | iters = " << iters << "\n";
